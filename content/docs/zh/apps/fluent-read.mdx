...
description: FluentRead 教程 — 开源浏览器翻译插件，支持 20+ 翻译引擎与 AI 大模型翻译。对接 BestTokenRouter 实现沉浸式双语阅读体验。
---
...
### 从 BestTokenRouter 控制台导入配置（推荐）

当浏览器安装了流畅阅读插件后，打开 BestTokenRouter 控制台->令牌管理页面会弹出添加流畅阅读的提示
...
确认导入后在流畅阅读中的BestTokenRouter配置便会启用
...
| 配置项     | 内容                       |
| ---------- | -------------------------- |
| 翻译服务   | BestTokenRouter                     |
| 访问令牌   | BestTokenRouter 密钥                |
| BestTokenRouter接口 | BestTokenRouter部署地址（不带/v1）  |
| 模型       | 列表中选择，或者自定义模型 |
| 自定义模型 | 模型名称                   |
