...
description: FluentRead 教程 — 开源浏览器翻译插件，支持 20+ 翻译引擎与 AI 大模型翻译。对接 BEST TOKEN 实现沉浸式双语阅读体验。
---
...
### 从 BEST TOKEN 控制台导入配置（推荐）

当浏览器安装了流畅阅读插件后，打开 BEST TOKEN 控制台->令牌管理页面会弹出添加流畅阅读的提示
...
| 翻译服务   | BEST TOKEN                     |
| 访问令牌   | BEST TOKEN 密钥                |
| BEST TOKEN接口 | BEST TOKEN部署地址（不带/v1）  |
| 模型       | 列表中选择，或者自定义模型 |
| 自定义模型 | 模型名称                   |
