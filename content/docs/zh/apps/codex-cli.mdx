...
description: OpenAI Codex CLI 安装教程 — 在 Windows、macOS、Linux 上安装终端 AI 编程助手，对接 BEST TOKEN。支持沙箱策略与原子补丁编辑。
---
...
需要创建两个配置文件来配置 Codex CLI 的 API 地址和密钥。

<Callout type="info" title="创建配置目录和文件">
  在 WSL 中执行以下命令（请将示例地址和密钥替换为你自己的值）：
...
<Callout type="info" title="写入 ~/.codex/config.toml">
  ```bash
  ...
  请将 `base_url` 替换为你的 API 地址，末尾需要带 `/v1`。
</Callout>

<Callout type="info" title="写入 ~/.codex/auth.json">
  ```bash
  ...
  请将 `OPENAI_API_KEY` 替换为你的 API Token（可在控制台令牌管理页面获取）。
</Callout>

<Callout type="info" title="设置文件权限（推荐）">
  ```bash
  chmod 700 ~/.codex
...
> 注意：修改接口地址后，使用所有模型（包括官方预设模型）均调用您配置的、合法授权的自有或组织接入点。请使用您自己部署的 BEST TOKEN，或确认服务方具备合法上游授权和合规义务的 BEST TOKEN 服务。不要将来源不明的 API 地址或密钥接入生产环境。
...
<Callout type="info" title="创建配置目录">
  ```bash
  mkdir -p ~/.codex
  ```
</Callout>
...
<Callout type="info" title="写入 ~/.codex/config.toml">
  使用任意文本编辑器创建 `~/.codex/config.toml`，写入以下内容（请将示例地址替换为你自己的值）：
