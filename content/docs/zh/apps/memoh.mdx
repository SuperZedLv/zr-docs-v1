...
description: Memoh 对接教程 — 将自托管容器化 AI 智能体平台对接 BestTokenRouter，为 Telegram、Discord、飞书、QQ、微信等渠道创建具有长期记忆的 AI 机器人。
---
...
Memoh 支持接入任何 OpenAI 兼容的模型提供商，可以通过 BestTokenRouter 来统一管理和访问各种 AI 模型服务。
...
登录 Memoh Web 管理面板，进入提供商管理页面，点击BestTokenRouter。

![switch-to-newapi-provider](/assets/memoh/switch_to_newapi_provider.png)

在配置页面中填写以下信息：

- **API Base URL**：填写 BestTokenRouter 的接口地址，例如 `https://api.example.com/v1`（如果本地部署则填写 `http://localhost:3000/v1`）
- **API Key**：粘贴从 BestTokenRouter 复制的 API Key 密钥
...
进入机器人设置页面，在模型配置一栏中，将默认聊天模型切换为刚刚通过 BestTokenRouter 提供商添加的模型，点击保存。

![set_chat_model](/assets/memoh/set_chat_model.png)

至此，您已经成功配置了 BestTokenRouter 作为 Memoh 的模型提供商。现在，您可以通过 Memoh 中的各个渠道（Telegram、Discord、飞书等）与 AI 机器人对话，所有请求将通过 BestTokenRouter 进行转发。
