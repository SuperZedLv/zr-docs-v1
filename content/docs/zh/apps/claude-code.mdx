---
title: Claude Code
description: Claude Code 教程 — 将 Anthropic 的终端编程助手对接 BEST TOKEN。支持代码理解、多文件编辑，集成 VS Code 与 JetBrains IDE。
---
...
为了让 Claude Code 连接到你的中转服务，需要设置以下环境变量：

| 变量名                | 说明             | 示例值                              |
| --------------------- | ---------------- | ----------------------------------- |
| `ANTHROPIC_BASE_URL`  | API 基础地址     | `https://your-api-site.com`         |
| `ANTHROPIC_AUTH_TOKEN` | API 密钥（Token） | `sk-xxx`（在控制台令牌管理页面获取）|

<Callout type="info" title="在 PowerShell 中设置环境变量">
  将下面的地址和 Token 替换为你自己的值，然后逐行执行：
  ...
  以上命令会将环境变量写入用户级别，永久生效。同时设置当前会话使其立即生效：
...
<Callout type="info" title="获取 Token">
  访问你的 API 站点控制台令牌管理页面（如 `https://your-api-site.com/console/token`），创建或复制你的 API Token。
</Callout>
...
为了让 Claude Code 连接到第三方中转服务，需要设置以下环境变量：
...
<Callout type="info" title="直接修改环境变量">
  打开终端，将下面的地址和 Token 替换为你自己的值，然后执行：
  ...
  如果你使用的是 bash，请将 `~/.zshrc` 替换为 `~/.bashrc`。
</Callout>
