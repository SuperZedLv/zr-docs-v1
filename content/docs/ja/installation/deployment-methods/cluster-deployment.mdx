...
このドキュメントでは、BEST TOKEN クラスターデプロイの詳細な設定手順とベストプラクティスを提供し、高可用性で負荷分散された分散システムの構築を支援します。
...
BEST TOKEN クラスターはマスター・スレーブアーキテクチャを採用しています：
...
services:
  new-api-master:
    ...
    environment:
      ...
      # - FRONTEND_BASE_URL=https://<your-besttoken-domain> # フロントエンドのベースURL、メール通知などの機能に使用
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
services:
  new-api-slave:
    ...
    environment:
      ...
      - TZ=Asia/Shanghai
      # 以下はオプション設定
      # - FRONTEND_BASE_URL=https://<your-besttoken-domain> # マスターノードと同じである必要があります
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
