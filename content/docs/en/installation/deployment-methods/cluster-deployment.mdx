...
This document provides detailed configuration steps and best practices for BEST TOKEN cluster deployment, helping you build a highly available, load-balanced distributed system.
...
The BEST TOKEN cluster adopts a master-slave architecture design:
...
services:
  new-api-master:
    ...
    environment:
      ...
      # - FRONTEND_BASE_URL=https://<your-besttoken-domain> # 前端基础 URL，用于邮件通知等功能
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
services:
  new-api-slave:
    ...
    environment:
      ...
      - TZ=Asia/Shanghai
      # 以下是可选配置
      # - FRONTEND_BASE_URL=https://<your-besttoken-domain> # 需与主节点相同
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
server {
    listen 80;
    server_name <your-besttoken-domain>;
