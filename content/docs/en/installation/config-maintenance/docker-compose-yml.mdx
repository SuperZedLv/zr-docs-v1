...
This document details the Docker Compose configuration options for BestTokenRouter, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the BestTokenRouter service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the BestTokenRouter service itself:
...
### BestTokenRouter Service Configuration
...
| `environment`    | Environment variable settings, used to configure BestTokenRouter behavior              |
...
BestTokenRouter supports various environment variable configurations. Here are some commonly used ones:
