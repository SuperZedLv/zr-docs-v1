...
This document details the Docker Compose configuration options for BEST TOKEN, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the BEST TOKEN service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the BEST TOKEN service itself:
...
### BEST TOKEN Service Configuration
...
| `environment`    | Environment variable settings, used to configure BEST TOKEN behavior              |
...
BEST TOKEN supports various environment variable configurations. Here are some commonly used ones:
