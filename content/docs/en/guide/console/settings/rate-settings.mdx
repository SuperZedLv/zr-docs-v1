...
Ratio settings are a core configuration of BestTokenRouter's internal cost accounting system. By setting different ratios, you can flexibly control internal cost allocation, budget control, or enterprise customer billing standards for various models and user groups.

## Ratio System Overview

BestTokenRouter uses a three-tier ratio system to calculate user quota consumption:
...
In the BestTokenRouter system, ratios are key parameters for calculating quota consumption. Quota is the internal billing unit of the system, and all API calls are ultimately converted into quota points for deduction.
...
#### Audio Models (Special handling, automatically processed internally by BestTokenRouter)
...
BestTokenRouter adopts a dual billing mechanism of pre-consumption and post-consumption:
