...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with BEST TOKEN to enable multi-model workflows.
---
...
Provider Type: Types supported by BEST TOKEN
API Key: Obtained from BEST TOKEN
API Address: BEST TOKEN site address (e.g., `https://<your-besttoken-domain>/v1`)

Please use your self-deployed BEST TOKEN, or ensure that the service provider offers BEST TOKEN services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in BEST TOKEN**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select BEST TOKEN
    ![Add BEST TOKEN Provider](/assets/aionui/besttoken_provider.png)

4.  **Configure API Information**
    - API Address: Enter your BEST TOKEN site address (format: `https://<your-besttoken-domain>/v1`)
    - API Key: Paste the API Key copied from the BEST TOKEN console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in BEST TOKEN
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured BEST TOKEN model to start a conversation
