...
description: Memoh integration guide — Connect the self-hosted containerized AI agent platform to BestTokenRouter, creating AI bots with long-term memory for Telegram, Discord, Lark, QQ, WeChat, and more.
---
...
## BestTokenRouter Integration

Memoh supports any OpenAI-compatible model provider, allowing you to use BestTokenRouter to centrally manage and access various AI model services.

### Configuration Steps

#### Obtain a BestTokenRouter API Key

After registering and logging in to BestTokenRouter, click "Console" in the top navigation bar, then click "Token Management", and click the "Add Token" button to create a new API Key. Select the appropriate permissions and click "Create".
...
Log in to the Memoh Web dashboard, go to the provider management page, and click BestTokenRouter.
...
- **API Base URL**: Enter the BestTokenRouter endpoint URL, e.g. `https://api.example.com/v1` (use `http://localhost:3000/v1` for local deployments)
- **API Key**: Paste the API Key copied from BestTokenRouter
...
Go to the bot settings page, find the model configuration section, switch the default chat model to the one added via the BestTokenRouter provider, and click Save.

![set_chat_model](/assets/memoh/set_chat_model.png)

You have now successfully configured BestTokenRouter as the model provider for Memoh. You can chat with AI bots through any of Memoh's channels (Telegram, Discord, Lark, etc.), and all requests will be routed through BestTokenRouter.
