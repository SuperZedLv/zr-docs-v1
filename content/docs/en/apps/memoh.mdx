...
description: Memoh integration guide — Connect the self-hosted containerized AI agent platform to BEST TOKEN, creating AI bots with long-term memory for Telegram, Discord, Lark, QQ, WeChat, and more.
---
...
## BEST TOKEN Integration

Memoh supports any OpenAI-compatible model provider, allowing you to use BEST TOKEN to centrally manage and access various AI model services.

### Configuration Steps

#### Obtain a BEST TOKEN API Key

After registering and logging in to BEST TOKEN, click "Console" in the top navigation bar, then click "Token Management", and click the "Add Token" button to create a new API Key. Select the appropriate permissions and click "Create".
...
Log in to the Memoh Web dashboard, go to the provider management page, and click BEST TOKEN.

![switch-to-besttoken-provider](/assets/memoh/switch_to_besttoken_provider.png)

Fill in the following information on the configuration page:

- **API Base URL**: Enter the BEST TOKEN endpoint URL, e.g. `https://api.example.com/v1` (use `http://localhost:3000/v1` for local deployments)
- **API Key**: Paste the API Key copied from BEST TOKEN
...
Go to the bot settings page, find the model configuration section, switch the default chat model to the one added via the BEST TOKEN provider, and click Save.

![set_chat_model](/assets/memoh/set_chat_model.png)

You have now successfully configured BEST TOKEN as the model provider for Memoh. You can chat with AI bots through any of Memoh's channels (Telegram, Discord, Lark, etc.), and all requests will be routed through BEST TOKEN.
