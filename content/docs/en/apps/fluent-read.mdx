...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to BEST TOKEN for immersive bilingual reading.
---

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the BEST TOKEN console, you can add
...
### Import Configuration from BEST TOKEN Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the BEST TOKEN console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the BEST TOKEN configuration in FluentRead will be enabled.
...
| ------------------- | --------------------------------------- |
| Translation Service | BEST TOKEN                                  |
| Access Token        | BEST TOKEN Key                              |
| BEST TOKEN Endpoint     | BEST TOKEN Deployment Address (without /v1) |
