---
title: Claude Code
description: Claude Code configuration guide — use Anthropic's terminal-based coding agent with BEST TOKEN. Supports code understanding, multi-file editing, and IDE integration.
---
...
To allow Claude Code to connect to your relay service, you need to set the following environment variables:

| Variable Name        | Description                | Example Value                        |
| -------------------- | -------------------------- | ------------------------------------ |
| `ANTHROPIC_BASE_URL` | API base URL               | `https://your-api-site.com`          |
| `ANTHROPIC_AUTH_TOKEN`| API key (Token)            | `sk-xxx` (obtain from console token management page) |

<Callout type="info" title="Set environment variables in PowerShell">
  Replace the URL and Token below with your own values, then execute line by line:

  ```powershell
  [Environment]::SetEnvironmentVariable('ANTHROPIC_BASE_URL', 'https://your-api-site.com', 'User')
  [Environment]::SetEnvironmentVariable('ANTHROPIC_AUTH_TOKEN', 'sk-xxx', 'User')
  ```

  The above commands write the environment variables at the user level, making them permanent. Also set them in the current session for immediate effect:

  ```powershell
  $env:ANTHROPIC_BASE_URL = 'https://your-api-site.com'
  $env:ANTHROPIC_AUTH_TOKEN = 'sk-xxx'
  ```
</Callout>

<Callout type="info" title="Obtain Token">
  Visit your API site's console token management page (e.g., `https://your-api-site.com/console/token`) to create or copy your API Token.
</Callout>
...
To allow Claude Code to connect to a third-party relay service, you need to set the following environment variables:
...
<Callout type="info" title="Directly modify environment variables">
  Open Terminal, replace the URL and Token below with your own values, then execute:

  ```bash
  echo 'export ANTHROPIC_BASE_URL="https://your-api-site.com"' >> ~/.zshrc
...
<Callout type="info" title="Directly modify environment variables">
  ...
  ```bash
  echo 'export ANTHROPIC_BASE_URL="https://your-api-site.com"' >> ~/.bashrc
