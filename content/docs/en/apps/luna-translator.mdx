...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with BEST TOKEN to improve translation quality.
---
...
## Integrating BEST TOKEN into LunaTranslator

LunaTranslator supports integration with locally deployed BEST TOKEN. Please use your own deployed BEST TOKEN, or ensure that the service provider has legitimate upstream authorization and compliance obligations for their BEST TOKEN service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the BEST TOKEN console's `System Settings` -> `Chat Settings`, add the following shortcut option:
...
2.  In the **`BEST TOKEN`** -> `Console` -> `Token Management` tab, select the token you want to use in LunaTranslator, click the dropdown option next to the chat button, choose `LunaTranslator`, and you will be redirected to LunaTranslator with the API address and API Key automatically configured.
...
4.  Click the refresh button next to the **model** dropdown to retrieve the BEST TOKEN platform's model list, select or enter the model name, then click OK to save.
...
1.  In the **`BEST TOKEN`** -> `Console` -> `Token Management` tab, obtain the API Key.
...
5.  Click the refresh button next to the **model** dropdown to retrieve the BEST TOKEN platform's model list, select or enter the model name, then click OK to save.

    ![设置 API3](/assets/luna_translator/setting_api3.png)

6.  Click the toggle switch next to **BEST TOKEN** to enable the interface and start using it.
