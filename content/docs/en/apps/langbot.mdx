...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with BEST TOKEN.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with BEST TOKEN.
</Callout>
...
## Integrating BEST TOKEN

LangBot supports integrating with locally deployed BEST TOKEN and third-party BEST TOKEN services built using BEST TOKEN.

### Usage

1.  Obtain an API key from BEST TOKEN
    ![获取 API key](/assets/langbot/get_api_key.png)

        If BEST TOKEN is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party BEST TOKEN service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select BEST TOKEN as the provider, and fill in the corresponding API key and API address
    ![添加 BEST TOKEN 模型](/assets/langbot/add_besttoken_model.png)
...
LangBot supports using BEST TOKEN's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select BEST TOKEN as the provider
