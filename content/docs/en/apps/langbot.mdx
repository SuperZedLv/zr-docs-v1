...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with BestTokenRouter.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with BestTokenRouter.
</Callout>
...
## Integrating BestTokenRouter

LangBot supports integrating with locally deployed BestTokenRouter and third-party BestTokenRouter services built using BestTokenRouter.

### Usage

1.  Obtain an API key from BestTokenRouter
    ![获取 API key](/assets/langbot/get_api_key.png)

        If BestTokenRouter is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party BestTokenRouter service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select BestTokenRouter as the provider, and fill in the corresponding API key and API address
    ![添加 BestTokenRouter 模型](/assets/langbot/add_newapi_model.png)
...
LangBot supports using BestTokenRouter's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select BestTokenRouter as the provider
